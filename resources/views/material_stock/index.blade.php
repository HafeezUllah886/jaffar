                        ...
                        <tbody>
                            @foreach ($products as $key => $product)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $product->name }}</td>
                                    <td>{{ number_format(getMaterialStock($product->id) / $product->unit->value, 2) }} {{$product->unit->name}}</td>
                                    <td>
...
@section('page-js')
    ...
    <script>
        ...
        $("#viewBtn").on("click", function (){
            ...
            var from = $("#from").val();
            var to = $("#to").val();
            var url = "{{ route('materialDetails', ['id' => ':productID', 'unit' => ':unit', 'from' => ':from', 'to' => ':to']) }}"
