                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody id="products_list">
                                        @foreach ($sale->details as $product)
                                            @php
                                                ...
                                                $batches = App\Models\stock::select('batchNumber', DB::raw('SUM(cr) - SUM(db) AS balance'))
                                            ...
                                            <tr id="row_{{ $id }}">
                                                ...
                                                <td class="no-padding">
                                                    <select class="form-control text-center" name="batchNumber[]" id="batch_{{ $id }}" onchange="updateChanges({{ $id }})">
                                                        @foreach ($batches as $b)
                                                        @php
                                                        if($b->batchNumber == $product->batchNumber)
                                                        {
                                                            $balance = $b->balance + $product->qty;
                                                            $b->balance = $balance;
                                                        }
                                                        @endphp
                                                        @if($balance > 0)
                                                            <option value="{{ $b->batchNumber }}" data-stock="{{ $b->balance }}" @selected($b->batchNumber == $product->batchNumber)>{{ $b->batchNumber }}</option>
                                                        @endif
                                                        @endforeach
                                                    </select>
                                                </td>
                                                ...
                                                <td class="no-padding">
                                                    <div class="input-group">
                                                        ...
                                                        required step="any" value="1" class="form-control text-center" id="qty_{{ $id }}">
                                                    </div>
                                                </td>
                                                <td class="no-padding">
                                                    <input type="number" name="price[]" oninput="updateChanges({{$id}})" required step="any"
                                                    value="{{ $product->price }}" min="1" class="form-control text-center" id="price_{{ $id }}">
                                                </td>
                                                <td class="no-padding">
                                                    <div class="input-group">
                                                        <input type="number" name="discount[]" oninput="updateChanges({{ $id }})" required
                                                        step="any" value="{{ $product->discount }}" min="0" class="form-control text-center"
                                                        id="discount_{{ $id }}">
                                                        <span class="input-group-text" id="discountValue_{{ $id }}">{{ $product->discountValue / $product->qty }}</span>
                                                    </div>
                                                </td>
                                                <td class="no-padding">
                                                    <div class="input-group">
                                                        <input type="number" name="scheme[]" oninput="updateChanges({{ $id }})" required
                                                        step="any" value="{{ $product->scheme }}" min="0" class="form-control text-center"
                                                        id="scheme_{{ $id }}">
                                                        <span class="input-group-text" id="schemeValue_{{ $id }}">{{ $product->schemeValue / $product->qty }}</span>
                                                    </div>
                                                </td>
                                                <td class="no-padding">
                                                    <input type="number" name="amount[]" required step="any" readonly value="{{ $product->amount }}"
                                                    class="form-control text-end" id="amount_{{ $id }}">
                                                </td>
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <th class="text-end" id="total">{{ $sale->details->sum('amount') }}</th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
...
@section('page-js')
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(product) {
                    ...
                    if (found.length > 0) {
                        ...
                        html += '<select class="form-control text-center" name="batchNumber[]" id="batch_' +
                            id + '" onchange="updateChanges(' + id + ')">';
                        product.batches.forEach(function(b) {
                            if (b.balance > 0) {
                                html += '<option value="' + b.batchNumber + '" data-stock="' + b
                                    .balance + '">' + b.batchNumber + '</option>';
                            }
                        });
                        html += '</select>';
                        html += '</td>';
                        html +=
                            '<td class="no-padding"><select name="unit[]" class="form-control text-center" onchange="updateChanges(' +
                            id + ')" id="unit_' + id + '">';
                        ...
                        html += '</select></td>';
                        html +=
                            '<td class="no-padding"><div class="input-group">  <span class="input-group-text" id="stockValue_' +
                            id + '">' + product.stock +
                        ...
                        html +=
                            ...
                            id +
                            ')" required step="any" value="' + product.price +
                            '" min="1" class="form-control text-center" id="price_' +
                            id + '"></td>';
                        html +=
                            ...
                            '" min="0" class="form-control text-center" id="discount_' +
                            id + '"> <span class="input-group-text" id="discountValue_' + id + '">' + product
                            .stock + '</span></div></td>';
                        html +=
                            '<td class="no-padding"><div class="input-group"> <input type="number" name="scheme[]" oninput="updateChanges(' +
                            id +
                            ')" required step="any" value="' + product.scheme +
                            '" min="0" class="form-control text-center" id="scheme_' +
                            id + '"> <span class="input-group-text" id="schemeValue_' + id + '">' + product
                            .stock + '</span></div></td>';
                        html +=
        ...
        function updateChanges(id) {
            ...
            console.log(qty, price, unit, stock, discount, scheme, newQty, discountValue, schemeValue, newPrice, amount);
